import cv2
import numpy as np

# Rangos HSV calibrados
rangos_hsv = {
    "Azul": ([96, 201, 178], [179, 255, 255]),
    "Verde": ([64, 180, 167], [99, 255, 248]),
    "Rojo": ([131, 38, 171], [179, 194, 255]),
    "Amarillo": ([66, 28, 226], [87, 77, 255]),
    "Naranja": ([3, 0, 203], [50, 110, 255]),
    "Carne": ([87, 29, 229], [99, 77, 255]),
}

def calcular_angulos(puntos):
    def angulo(p1, p2, p3):
        v1 = p1 - p2
        v2 = p3 - p2
        cos_ang = np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2))
        return np.degrees(np.arccos(np.clip(cos_ang, -1.0, 1.0)))
    angs = []
    for i in range(len(puntos)):
        p1 = puntos[i - 1][0]
        p2 = puntos[i][0]
        p3 = puntos[(i + 1) % len(puntos)][0]
        angs.append(angulo(p1, p2, p3))
    return angs

def pendiente(p1, p2):
    if p2[0] - p1[0] == 0:
        return float('inf')
    return (p2[1] - p1[1]) / (p2[0] - p1[0])

def detectar_forma(approx, area, perimetro):
    vertices = len(approx)
    approx = approx.reshape((vertices, 2))

    if vertices == 3:
        return "TRIA"

    elif vertices == 4:
        pts = approx
        lados = [np.linalg.norm(pts[i] - pts[(i + 1) % 4]) for i in range(4)]
        angs = calcular_angulos(approx.reshape((-1, 1, 2)))
        pendientes = [pendiente(pts[i], pts[(i + 1) % 4]) for i in range(4)]
        paralelos = 0
        umbral_paralelos = 0.35
        if abs(pendientes[0] - pendientes[2]) < umbral_paralelos:
            paralelos += 1
        if abs(pendientes[1] - pendientes[3]) < umbral_paralelos:
            paralelos += 1

        angulos_rectos = all(83 <= a <= 97 for a in angs)
        lados_opuestos_similares = abs(lados[0] - lados[2]) < 15 and abs(lados[1] - lados[3]) < 15

        x, y, w, h = cv2.boundingRect(approx)
        aspect_ratio = float(w) / h

        # Detectar rectángulo (incluye cuadrados deformados)
        if angulos_rectos and paralelos == 2 and 0.6 <= aspect_ratio <= 1.7:
            return "CUADRADO"

        # Si solo un par de paralelos, verificar si realmente no es rectángulo
        if paralelos == 1:
            # Si ángulos son casi rectos y lados opuestos similares, prioriza rectángulo
            if angulos_rectos and lados_opuestos_similares:
                return "CUADRADO"
            return "TRAP"

        # Detección de rombos (lados iguales y ángulos específicos)
        lados_iguales = max(lados) - min(lados) < 10
        min_ang = min(angs)
        max_ang = max(angs)
        if lados_iguales:
            if 55 <= min_ang <= 65 and 115 <= max_ang <= 125:
                return "ROMBO 1"
            elif 25 <= min_ang <= 35 and 145 <= max_ang <= 155:
                return "Rombo 2"
            else:
                return "Rombo"

    elif vertices == 6:
        return "HEX"

    return "UNKNOWN"

cv2.namedWindow("Original")
cv2.namedWindow("Preprocesado")
cv2.namedWindow("Resultado")

cap = cv2.VideoCapture(0)

while True:
    ret, frame = cap.read()
    if not ret:
        break

    frame = cv2.resize(frame, (320, 240))
    original = frame.copy()
    resultado = frame.copy()

    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    for color, (bajo, alto) in rangos_hsv.items():
        bajo = np.array(bajo)
        alto = np.array(alto)
        mascara = cv2.inRange(hsv, bajo, alto)

        # Filtros morfológicos para limpiar la máscara
        kernel = np.ones((5, 5), np.uint8)
        mascara = cv2.morphologyEx(mascara, cv2.MORPH_OPEN, kernel)
        mascara = cv2.morphologyEx(mascara, cv2.MORPH_CLOSE, kernel)

        # Filtros de suavizado
        mascara = cv2.GaussianBlur(mascara, (3, 3), 1.2)
        mascara = cv2.medianBlur(mascara, 3)

        contornos, _ = cv2.findContours(mascara, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        for cnt in contornos:
            area = cv2.contourArea(cnt)
            if area > 1000:
                perimetro = cv2.arcLength(cnt, True)
                approx = cv2.approxPolyDP(cnt, 0.03 * perimetro, True)
                forma = detectar_forma(approx, area, perimetro)
                x, y, w, h = cv2.boundingRect(approx)
                cv2.drawContours(resultado, [approx], -1, (0, 255, 0), 2)
                cv2.putText(resultado, f"{forma} {color}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                cv2.rectangle(resultado, (x, y), (x + w, y + h), (255, 0, 255), 1)

    preprocesado = cv2.cvtColor(mascara, cv2.COLOR_GRAY2BGR)
    cv2.imshow("Original", original)
    cv2.imshow("Preprocesado", preprocesado)
    cv2.imshow("Resultado", resultado)

    if cv2.waitKey(1) & 0xFF == ord('s'):
        break

cap.release()
cv2.destroyAllWindows()
